...
class ActivateController extends Controller
{
    ...
    public function activate($token)
    {
        ...
        // for soap command to register a user
        $flavour = Flavour::where('username', Auth::user()->username)->first();
        $created = $this->accountCreate($flavour);
        $flavour->delete();
        ...
        return redirect()->route(self::getDownloadRoute())
            ->with('status', 'success')
            ->with('message', trans('auth.successActivated'));
    }
    ...
    private function accountCreate($userInfo)
    {
      $client = new Client('test','test','95.216.150.5','7880');
      $responds = $client->account()->create($userInfo->username, $userInfo->flavour);

      return  (isset($responds['status'])? FALSE : $responds[0]);
    }
}
